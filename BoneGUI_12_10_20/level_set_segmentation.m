...
%------------------------------------------------------------------------
function [seg aux_seg]= level_set_segmentation(I,init_mask,max_its,min_its,alpha)
...
%--main loop
temp=zeros(1,min_its);
temp(1,min_its)=10;
aux_temp=0;
its=1;
iszero_seg=false;

while (its <=max_its && aux_temp < min_its && ~iszero_seg)
    ...
    dphidt = F./(max(abs(F))+eps) + alpha*curvature;  % gradient descent to minimize energy
    ...
    if its <2
        ...
        temp(1)=[];
        temp(min_its)=sum(sum(seg));
        aux_temp=sum(abs(temp-temp(1))<=2);
        if temp(min_its)==0
            iszero_seg=true;
        end
    end
    aux_seg(its)=sum(sum(seg));
    its=its+1;
end
